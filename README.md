...
実際にサーバーで公開しているので、下記URLでアクセスできます
パスワードは御社の名前の英語の綴りとなっています。
どうしてもセキュリティは初学者なこともあり、危険かもしれません　ある程度気をつけてる
...
-   **NVIDIA製GPU** と、対応するグラフィックドライバ　ローカルllm使うなら4060と同等かそれ以上は必要
...
表情や動作をllmに委ねるとどうしてもラグが発生してしまう　両立が大事
12Bもあるが、結局遅延を考えると4Bが妥当

ただ、知識がない状態で生成AIに委ねることは大変だった。頼りすぎるのもよくない。
頼りすぎたのは課題
やはり現場ではセキュリティ等で生成AIが使えないこともあるので、自信である程度はコードがかけるようになるという必要性は実感した
生成AIだとどうしてもwebuiがズレてしまう。いずれは完璧になるだろうが、
エラーがなぜ発生し、なぜ解決したかがわからない場面も多々あった。
実際に実装は大変苦戦した。現場の気持ちがわかったような気がする
